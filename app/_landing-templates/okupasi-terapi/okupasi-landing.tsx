// Template LANDING untuk klinik bertipe `okupasi_terapi`.
// Tampilan terinspirasi gaya "Play Kids" — ceria, kaya gambar & ikon warna-warni,
// dengan section Cara Booking + FAQ + tombol WhatsApp mengambang. Tema indigo/violet.
...
import {
  ...
  Brain, Baby, Accessibility, Shapes, Utensils, PenTool, Blocks,
  Clock, Quote, MapPin, Phone, MessageCircle, CalendarCheck, Smile,
...
import { OkupasiNav, OkupasiFooter } from "./chrome";
import { Reveal } from "../fisioterapi/reveal";
import { OKU_IMG } from "./defaults";
...
const FEATURE_ICONS = [HandHeart, Brain, Sparkles, Clock, Smile, Shapes];
const DEFAULT_FEATURES = [
  { title: "Okupasi Terapis Bersertifikat", description: "Ditangani okupasi terapis profesional ber-STR dan berpengalaman." },
  { title: "Program Individual", description: "Rencana terapi disusun sesuai kebutuhan, usia, dan target kemandirian." },
  { title: "Ruang Sensori & Bermain", description: "Didukung alat terapi sensori-motorik dan ruang bermain yang aman." },
  { title: "Pendampingan Keluarga", description: "Edukasi dan home program agar terapi berlanjut di rumah." },
];

const DEFAULT_SERVICES = [
  { name: "Terapi Sensori Integrasi", description: "Membantu anak memproses & merespons rangsang sensorik dengan tepat.", icon: Brain },
  { name: "Stimulasi Motorik Halus", description: "Melatih koordinasi tangan untuk menulis, menggunting, dan menggenggam.", icon: PenTool },
  { name: "Latihan Aktivitas Sehari-hari", description: "Melatih makan, berpakaian, dan mandi agar lebih mandiri.", icon: Utensils },
  { name: "Terapi Tumbuh Kembang Anak", description: "Mendukung perkembangan anak dengan keterlambatan atau hambatan.", icon: Baby },
  { name: "Okupasi Terapi Dewasa", description: "Pemulihan fungsi & kemandirian pasca stroke, cedera, atau penyakit.", icon: Accessibility },
  { name: "Terapi Bermain & Perilaku", description: "Mengembangkan fokus, interaksi sosial, dan regulasi emosi.", icon: Blocks },
];

const SERVICE_ICONS = [Brain, PenTool, Utensils, Baby, Accessibility, Blocks];
...
const DEFAULT_TESTIMONIALS = [
  { name: "Ibu Sari", role: "Orang tua pasien anak", text: "Anak saya jadi lebih fokus dan mau mencoba hal baru. Terapisnya sabar dan komunikatif dengan keluarga." },
  { name: "Bpk. Andi", role: "Pemulihan pasca-stroke", text: "Saya kembali bisa makan dan berpakaian sendiri. Latihannya bertahap dan terasa hasilnya." },
  { name: "Ibu Lina", role: "Orang tua pasien", text: "Suasana kliniknya ramah anak. Setiap sesi ada laporan progres yang jelas, jadi tenang." },
];

const DEFAULT_FAQS = [
  { q: "Usia berapa anak bisa mulai terapi?", a: "Kami menangani anak sejak dini. Semakin dini kebutuhan terdeteksi dan ditangani, semakin optimal hasilnya. Bila ragu, mulailah dengan asesmen tumbuh kembang." },
  { q: "Apakah perlu rujukan dokter terlebih dahulu?", a: "Tidak wajib. Anda bisa langsung booking sesi asesmen awal. Bila ada hasil pemeriksaan sebelumnya, silakan dibawa agar asesmen lebih akurat." },
...
export default function OkupasiLanding({ clinic, content, services, therapists, bookingOpen }: LandingData) {
  ...
  const heroSubtitle =
    ...
    (isMulti
      ...
      : "Mendampingi anak dan dewasa mencapai kemandirian dalam aktivitas sehari-hari melalui okupasi terapi yang personal, menyenangkan, dan terukur.");
  const tagline = content.tagline || (isMulti ? `Klinik ${svcList}` : "Klinik Okupasi Terapi Terpercaya");
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white">
      <OkupasiNav clinic={clinic} bookingOpen={bookingOpen} />
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-4 py-16 lg:grid-cols-2 lg:py-24">
          <Reveal>
            {bookingOpen ? (
              ...
              <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-100 bg-indigo-50 px-4 py-1.5 text-sm font-semibold text-indigo-700">
                <HandHeart className="h-4 w-4" /> {tagline}
              </div>
            )}
      ...
      <section id="layanan" className="mx-auto w-full max-w-6xl px-4 py-20">
        <SectionTitle kicker="Layanan Kami" icon={Sparkles} title={isMulti ? "Layanan Terapi Lengkap" : "Penanganan Okupasi Terapi Lengkap"} desc={isMulti ? `Klinik kami melayani ${svcList} untuk mendukung pemulihan, kemandirian, dan tumbuh kembang anak maupun dewasa.` : "Berbagai layanan terapi untuk mendukung kemandirian, fungsi, dan tumbuh kembang anak maupun dewasa."} />
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-14 px-4 lg:grid-cols-2">
          ...
          <Reveal delay={120}>
            ...
            <h2 className="mt-4 text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">
              {content.founded_year ? `Mendampingi kemandirian sejak ${content.founded_year}` : "Tempat yang aman & menyenangkan untuk berkembang"}
            </h2>
            <p className="mt-4 text-gray-600">
              {content.about_text ||
                (isMulti
                  ? `${clinic.name} menghadirkan layanan ${svcList} dalam satu tempat. Dengan tim terapis profesional dan pendekatan yang personal serta menyenangkan, kami mendampingi proses perkembangan dan pemulihan anak maupun dewasa secara menyeluruh.`
                  : `${clinic.name} hadir untuk membantu setiap individu mencapai kemandirian dalam aktivitas sehari-hari. Dengan tim okupasi terapis profesional dan pendekatan yang personal serta menyenangkan, kami mendampingi proses perkembangan dan pemulihan secara menyeluruh.`)}
            </p>
      ...
      {therapists.length > 0 && (
        <section id="terapis" className="bg-gray-50/70 py-20">
          <div className="mx-auto max-w-6xl px-4">
            <SectionTitle kicker="Tim Profesional" icon={ShieldCheck} title={isMulti ? "Ditangani Terapis Bersertifikat" : "Ditangani Okupasi Terapis Bersertifikat"} desc="Tim kami berpengalaman menangani anak & dewasa dengan penuh kesabaran dan kehangatan." />
          ...
          <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-indigo-600 to-violet-600 px-8 py-14 shadow-2xl shadow-indigo-900/30 animate-gradient md:px-14">
            ...
            <div className="relative grid items-center gap-10 md:grid-cols-2">
              <div>
                ...
                <h2 className="mt-4 text-2xl font-extrabold leading-tight text-white sm:text-3xl">Siap memulai perjalanan kemandirian?</h2>
                <p className="mt-4 max-w-md text-sm text-indigo-50/90 sm:text-base">
                  Jangan tunda. Jadwalkan sesi okupasi terapi pertama Anda sekarang. Tim kami siap mendampingi setiap langkah perkembangan.
                </p>
      ...
      <OkupasiFooter clinic={clinic} content={content} />
